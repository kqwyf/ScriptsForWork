...
- `cancel`：使用任务名或任务ID取消任务。附带bash和zsh的补全功能，可使用Tab补全任务名和任务ID。
  ...
  **只要任务名称匹配，即便存在多个同名任务也将全部被取消。**

  **在极少情况下可能存在任务名与任务ID冲突的情形（任务A的任务ID与任务B的任务名相同），在此情况下若指定的关键字中包含该ID（名），则任务A与任务B均会被取消。**
  ...
  - 使用示例：
    - `cancel wsj_train wsj_prep 21807777`
      ...
      本例将取消任务名为`wsj_train`和`wsj_prep`的任务，以及任务ID为`21807777`的任务（或任务array）。
    - `cancel ALL`
